package com.ccnu.mobilebank.service.util;
...
public class TokenUtil {
    ...
    public static String generateToken(Long mobileId,Long personId) throws Exception{
...
//    过期的返回特殊状态码555，前端可以根据状态码进行进一步操作，也就是刷新token，达到无感操作
    public static List<Long> verifyToken(String token){
        try {
...
//            取出keyId，也就是获取userId
            ...
            Long personId = jwt.getClaim("personId").asLong();
//            类型转换
            List<Long> mobileAndPerson = new ArrayList<>();
            mobileAndPerson.add(Long.valueOf(mobileId));
